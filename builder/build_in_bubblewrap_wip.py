...
def build(h: str, pkg_drvs: dict, pkg_names: dict, pkghash2sysroothash: dict) -> None:
    ...
    if symlink_build_inputs or (not sandboxed):
        ...
        if (not EXP_OVERLAYFS):
            ...
            sysroot_unshare = workdir + "/sysroot/"

            unshare_wrapper_script = sysroot_utils.build_overlay_sysroot( workdir, drv["buildInputDrvs"], uses_ccache,sandboxed )
            print(unshare_wrapper_script)
            # exit()
    ...
    with open(f"{workdir}/b.log", "a", encoding="utf-8") as f:
        with open(f"{workdir}/error.log", "a", encoding="utf-8") as f_error:
            ...
            if (not sandboxed) or symlink_build_inputs or (not EXP_OVERLAYFS):
                final_args = bwrap_wrap
            else:
                unshare_wrapper_fp = f"{workdir}/wrap-build.sh"
                with open(unshare_wrapper_fp,"w", encoding="utf-8") as f:
                    f.write(unshare_wrapper_script)
                subprocess.run(["chmod","+x",unshare_wrapper_fp],check=True)
                subprocess.run(["mkdir",  f"{sysroot_unshare}proc"])
                final_args =   [ "unshare",  "--map-root-user", "--mount" ,"--pid", "--fork", unshare_wrapper_fp , "/tmp/workdir/build.sh"]

...
extern void *dlopen_libc(const char *, int);
extern void *dlsym_libc(void *__restrict, const char *__restrict);
extern int dladdr_libc(const void *handle, Dl_info *info);
...
int dladdr(const void *handle, Dl_info *info)
{
    ...
    return dladdr_libc(handle, info);
}

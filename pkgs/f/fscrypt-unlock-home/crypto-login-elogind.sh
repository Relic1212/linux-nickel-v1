...
SUMMANDS_HEREDOC_INPUT
    ...
    if [ "$is_locked" = "0" ]; then
        echo "already unlocked, logging in"
        echo running /usr/bin/login "${USER_NAME}"

        user_session=$(/usr/bin/busctl call org.freedesktop.login1 /org/freedesktop/login1 org.freedesktop.login1.Manager CreateSession 'uusssssussbssa(sv)' 1000 0 'login' 'tty' 'user' '' '' 1 'tty1' '' false '' '' 0 -j --no-pager | jq .[\"data\"][0] | cut -d '"' -f2)
        echo "session=$user_session"

        /usr/bin/login "${USER_NAME}"
        echo "status is $?"
        /usr/bin/busctl call org.freedesktop.login1 /org/freedesktop/login1 org.freedesktop.login1.Manager ReleaseSession s $user_session
        sleep 5
        exit
    fi
fi
...
if [ $unlock_status -eq 0 ]; then
    ...
    user_session=$(/usr/bin/busctl call org.freedesktop.login1 /org/freedesktop/login1 org.freedesktop.login1.Manager CreateSession 'uusssssussbssa(sv)' 1000 0 'login' 'tty' 'user' '' '' 1 'tty1' '' false '' '' 0 -j --no-pager | jq .[\"data\"][0] | cut -d '"' -f2); echo "session=$user_session"

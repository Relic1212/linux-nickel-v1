...
const char* lib__home_user__config_elephant_providers_desktopapplications_so = "lib__home_user__config_elephant_providers_desktopapplications_so";

void* dlopen(const char *path, int mode) {
	if (path == NULL) { return &main_program_handle; }
	if (strcmp( path, "/home/user/.config/elephant/providers/desktopapplications.so" ) == 0) { dbg_print("(dlopen) found library /home/user/.config/elephant/providers/desktopapplications.so (handle=%p)\n", &lib__home_user__config_elephant_providers_desktopapplications_so); return &lib__home_user__config_elephant_providers_desktopapplications_so; }
...
void* dlsym(void *__restrict handle, const char *__restrict symbol) {
	if (handle == &lib__home_user__config_elephant_providers_desktopapplications_so || handle == NULL || handle == &main_program_handle) { 
	}
